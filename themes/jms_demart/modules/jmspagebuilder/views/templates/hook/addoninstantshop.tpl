...
<div class="instant-tabs">
	<ul class="nav nav-tabs" role="tablist">
		...
		{foreach from = $categories key = k item = category}
			<li {if !$showall && $k ==0}class="active"{/if}>
				<a class="instant-btn" data-id="{$category.id_category|escape:'html':'UTF-8'}">{$category.name|escape:'htmlall':'UTF-8'}</a>
			</li>					
		{/foreach}	
	</ul>
	<div class="instantshop-filters btn-group">
		<a class="dropdown-toggle" data-toggle="dropdown">
			{l s='Filter' mod='jmspagebuilder'}
...
<div class="row" id="instant-products">
	{foreach from = $products item = product}				
		<div class="item col-6 col-sm-6 col-md-4 col-lg-{$col_class}">
